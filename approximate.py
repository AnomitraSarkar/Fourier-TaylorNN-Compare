...
from manim import *
import random
from models import SimpleNN, SkipConn, Fourier, ConvNN, TaylorNN, FourierTaylor
...
def LearnCurve( scene,
        ...
        for epoch in range(epochs):
            ...
            loss_val.append([epoch, running_loss/num_samples])
            scheduler.step()
        with open(losssaverfile, "w", newline="") as f:
            csv_writer = writer(f)
            csv_writer.writerows(loss_val)
...
class LearnSimple(Scene):
    def construct(self):
        net = SimpleNN(hidden_size=50, hidden_layers=7)
        ...
        def target_function(x):
            ...
            b = -2.3 
            a = -2.6
            return k + (k-a)*x - (k-b)*x**2 + u*np.sin(x+a) + v*np.cos(x+b) + w*np.sin(2*(x+c)) + p*np.cos(2*(x+d)) + q*np.sin(3*(x+f)) + s*np.cos(3*(x+g))
...
class LearnFourier(Scene):
    def construct(self):
        ...
        LearnCurve(self, target_function, net,
                    ...
                    sched_step=10,
                    smooth=False,
                    losssaverfile="fourierloss.csv")
        
class LearnFourierTaylor(Scene):
    def construct(self):
        net = FourierTaylor(fourier_order=8, taylor_order=3, hidden_size=100, hidden_layers=7)
        ...
        LearnCurve(self, target_function, net,
                    ...
                    sched_step=10,
                    smooth=False,
                    losssaverfile="fouriertaylorloss.csv")
...
class FourierOverfit(Scene):
    def construct(self):
        ...
        LearnCurve(self, target_function, net,
                    ...
                    sched_step=15,
                    smooth=False,
                    losssaverfile="fourierlossdefaultoverfit.csv")
class FourierTaylorOverfit(Scene):
    def construct(self):
        net = FourierTaylor(fourier_order=16, taylor_order=3, hidden_size=50, hidden_layers=5)
        ...
        LearnCurve(self, target_function, net,
                    ...
                    sched_step=15,
                    smooth=False,
                    losssaverfile="fouriertaylorlossdefaultoverfit.csv")
