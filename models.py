...
class FourierTaylor(nn.Module):
	def __init__(self, in_size=1, out_size=1, taylor_order=3, fourier_order=4, hidden_size=100, hidden_layers=7):
		super(FourierTaylor,self).__init__()
		self.fourier_order = fourier_order
		self.taylor_order = taylor_order
		in_size = (in_size * 2 * fourier_order) + (in_size) + (in_size * taylor_order)
		self.inner_model = SkipConn(in_size=in_size, out_size=out_size, hidden_size=hidden_size, hidden_layers=hidden_layers)
		self.orders_fourier = torch.arange(1, fourier_order + 1).float()
		self.orders_taylor = torch.arange(1, taylor_order + 1).float()

	def forward(self,x):
		x = x.unsqueeze(-1)  # add an extra dimension for broadcasting
		custom_features = torch.cat([torch.sin(self.orders_fourier * x), torch.pow(x,self.orders_taylor), torch.cos(self.orders_fourier * x),  x], dim=-1)
		custom_features = custom_features.view(x.shape[0], -1)  # flatten the last two dimensions
		return self.inner_model(custom_features)
